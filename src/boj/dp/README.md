...
정수 N이 주어졌을 때, 
1, 2, 3 의 합으로 N을 만드는 모든 경우의 수를 구하는 문제
(순서가 다른 경우는 서로 다른 경우)

## 접근 방법
- dp[n] = 정수 n을 1, 2, 3의 합으로 만드는 경우의 수
- 점화식: dp[n] = dp[n-1] + dp[n-2] + dp[n-3]
- 초기값: dp[1] = 1, dp[2] = 2, dp[3] = 4
- 경계값 처리: int[] dp = new int[Math.max(N + 1, 4)]
  (입력 N이 3이하일 경우)

## 시간 복잡도
O(N)

# BOJ 15988 - 1, 2, 3 더하기3

정수 n이 주어졌을 때,
1, 2, 3의 합으로 n을 만드는 모든 경우의 수를 구한다.
(결과는 1,000,000,009로 나눈 나머지를 출력)

## 접근 방법
- 결과 값이 매우 커질 수 있기 때문에 나머지 계산을 매 계산마다 적용
- 테스트 케이스가 많기 때문에 최대(1,000,000)까지 DP를 한번만 미리 계산
...
정수 n이 주어졌을 때,
1, 2, 3의 합으로 n을 만드는 경우의 수를 구하는 문제
(단, 순서가 다른 경우는 같은 경우로 계산)

## 접근 방법
dp[n][k]
- k = 1: 1만 사용해서 n을 만드는 경우
- k = 2 : 1, 2만 사용해서 n을 만드는 경우
- k = 3 : 1, 2, 3을 사용해서 n을 만드는 경우
* 마지막에 사용한 수를 기준으로 dp를 나누어 중복되는 순열 케이스를 제거
...
정수 n이 주어졌을 때,
1, 2, 3의 합으로 n을 만드는 경우의 수를 구한다.
결과는 1,000,000,009로 나눈 나머지를 출력하는 문제
(단, 같은 수를 연속해서 사용할 수 없음)

## 접근 방법
dp[n][k]
- n을 만드는 경우 중 마지막에 k를 사용한 경우의 수

- 같은 숫자를 연속으로 사용할 수 없으므로 이전 상태에서 자기 자신을 제외한 경우만 더한다.
...
## 접근 방법
- fib(n) 호출 시 0과 1이 출력되는 횟수를 dp로 저장
