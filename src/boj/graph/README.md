...
2차원 지도에서 1은 땅, 0은 바다를 의미한다.
상하좌우 + 대각선(총 8방향)으로 연결된 땅들은 하나의 섬으로 간주할 때,
지도에 존재하는 섬의 개수를 구하는 문제

## 접근 방법
- 전체 지도를 순회하며 아직 방문하지 않은 땅 발견 시 해당 위치를 시작점으로 BFS 탐색 수행
- BFS를 통해 연결된 모든 땅을 방문 처리하면 하나의 섬 탐색 완료로 판단
- h: 행(row), w: 열(col)
- Queue에 넣는 순간 방문 처리하여 중복 탐색 방지
...
## 접근 방법
- 전체 지도를 순회하며 아직 방문하지 않은 땅을 발견하면 DFS 수행
- DFS를 통해 연결된 땅을 끝까지 탐색하여 하나의 섬으로 처리

## 시간 복잡도
- O(h*w)

# BOJ 1697- 숨바꼭질
정수 N에서 시작하여 -1, +1, *2 연산만을 사용해 
정수 K에 도달하는 데 걸리는 최소 시간을 구하는 문제

## 접근 방법
- 각 숫자 위치를 하나의 정점(Node)으로 보고 그래프 탐색 문제로 해석
- DFS는 탐색 순서에 따라 최단 시간을 보장하지 못하기 때문에 최단 거리 탐색에 적합한 BFS 사용
- distance[x] 배열을 통해 시작점 N에서 x까지 도달하는데 걸린 최소 시간 저장
