...
## 문제 요약
주어진 N개의 수 중에서 중복을 허용하여 길이가 M인 수열을 모두 구하기
(단, 같은 수열은 한 번만 출력하며 출력은 사전 순(오름차순))

## 해결 방법
입력으로 받은 숫자 배열을 정렬한 뒤 DFS(백트래킹)를 사용
각 깊이(depth)마다 이전에 사용한 값(prev)을 저장하여 같은 깊이에서 같은 숫자가 다시 선택되지 않도록 하여 중복 수열을 제거
방문 배열은 사용하지 않으며 중복 제거는 prev 변수 하나로 처리
...
## 문제 요약
주어진 N개의 수 중에서 중복을 허용하여 길이가 M인 수열을 만들기
(단, 비내림차순(오름차순 유지) 수열만 출력하며, 같은 수열은 한 번만 출력)

## 해결 방법
입력받은 숫자 배열을 정렬한 후 DFS(백트래킹)를 사용
이전 인덱스(last)를 전달하여 현재 선택한 숫자보다 앞선 숫자를 다시 선택하지 않음으로써 비내림차순을 유지
같은 depth에서 중복 수열 생성을 막기 위해 prev 변수를 사용
